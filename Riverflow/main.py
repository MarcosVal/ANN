...
sheet = "Sheet3"  # clean database to train this MLP

rg = np.random.default_rng()
df = pd.read_excel('dataframe.xlsx', sheet_name=['Sheet1', 'Sheet2', 'Sheet3', 'Sheet4'])
...
def backward_pass(row):
    ...
    for node in range(hiddenNodes):  # update weights going to the output and hidden node biases
        ...
        for predictor in range(inputNum):
            ...
            weightHidden[predictor][node] = weightHidden[predictor][node]+(lr * delta[node] * stanData[row][predictor])\
                                            + (0.9*whDiff[predictor][node])
...
if __name__ == '__main__':
    ...
    # now with our predictor mins and maxs of the validation set we can standardise each column
    stanData = []
